...
const BlogsComponent = () => {
  return (
    <div className="w-full min-h-screen py-25 px-25 ">
      {blogsdetails.map((blog) => (
        <div className="w-full h-full flex flex-col gap-8">
          <img src={blog.image} alt="" className="w-full h-full rounded-4xl" />
          <div className="w-full h-full flex flex-col gap-10 px-30">
            ...
            <div className="w-full h-44 bg-indigo-950 rounded-4xl flex  justify-center p-7 tracking-tight gap-5">
              <img className="-mt-25" src={quoteImage} />{" "}
              <h2 className="text-white text-2xl">{blog.quote}</h2>
            </div>
            ...
            <div className="w-full h-full flex flex-col gap-5">
              ...
              <ul className="w-full flex flex-col gap-5 ml-7">
                {blog.lists.map((lis) => (
                  <li className="list-disc text-xl text-gray-500 opacity-90">
                    {lis}
                  </li>
                ))}
              </ul>

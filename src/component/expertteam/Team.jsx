...
const Team = ({setHovered,setContent}) => {
  ...
  return (
    <div className="w-full min-h-screen flex flex-col justify-between items-center mb-20">
        ...
        >
           {
            ourteam.map((team)=>(
             ...
             >
               <img  className ="w-full h-full group-hover:scale-110 transition duration-500" src={team.image}/>
               ...
               <div className="w-full h-full absolute bg-white/30 inset-0  scale-0 group-hover:scale-110 transition duration-500">
                   <div className="flex  space-x-5 items-center justify-center h-full cursor-pointer">
                        <span className="w-10 h-10 rounded-full border-[1px] border-white flex items-center justify-center"><FaXTwitter className="text-lg text-[#FFFFFF] "/></span>
                        <span className="w-10 h-10 rounded-full border-[1px] border-white flex items-center justify-center"> <FaFacebookF className="text-lg text-[#FFFFFF] "/></span>
                         <span className="w-10 h-10 rounded-full border-[1px] border-white flex items-center justify-center"> <FaInstagram className="text-lg text-[#FFFFFF] "/></span>
                        </div>
                </div>
             </div>
            ))
      ...
      <div className="max-w-7xl h-96 bg-gradient-to-r bg-indigo-600 via-35% to-blue-400 mt-65 flex  justify-between px-15 py-20">
        <div className="flex flex-col flex-2 justify-between">
          <div className="flex  justify-start">
            <h1 className="text-5xl text-white font-semibold">Protect business, cyber security solution today!</h1>
          </div>
          <div className="flex gap-10 items-center mt-5">
            <h1 className="text-white text-3xl flex items-center gap-3"><FaPhoneAlt/> Get contact now</h1>
            <span className="w-[2px] h-10 bg-gray-500 opacity-50"></span>
            <h1 className="text-white text-3xl flex items-center gap-3"><MdOutlineAttachEmail /> Sent e-mail</h1>
            <div className="absolute right-2/5 -mt-20">
              <img className="rotate-y-180" src={ctaArrowImage}/>
            </div>
          </div>
          <div>
          </div>
        </div>
        <img src={ctaImage} alt="" className="h-[35vh] flex-1 w-full rounded-3xl"/>
      </div>
    </div>
  );
};
